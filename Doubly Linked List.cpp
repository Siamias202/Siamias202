...
void printList_backward_recursive_approach(node *p)
{
	...
	printList_backward_recursive_approach(p->prev);
	cout<<p->data<<" ";
}

void reverse_iterative_approach()
{
	node *cur,*nextnode=NULL,*tail;
	cur=root;
	while(cur!=NULL)
	{
		nextnode=cur->prev;
...
void reverse_recursive_approach()
{

}

void append_ascending(ll x)
{
	node *temp=root;
	node *newnode=createnode(x);
	if(root==NULL)
...
int main()
{
   ...
   append_decending(4);
   append_decending(-2);
   printList_forward_iterative_approach();
   delete_node_value(4);
   printList_forward_iterative_approach();
   delete_node_index(1);
   printList_forward_iterative_approach();
